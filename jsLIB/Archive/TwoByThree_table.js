...
BERING.questionaire = (function () {
    ...
    function uiBE() {
        ...
        if (BERING.begining || BERING.end) {
            if ($("body").find("div.commentDisplay").children().length === 0) {
                if (BERING.begining) {
                    var msg = "<h3>Start here</h3><p>" + BERING.intro + "</p>";
                } else {
    ...
    function fairRepresentation(i) {
        ...
        var ret = "<span style='font-size: xx-large'>"+chartText+"</span>";
        if (chartText !== inventoryText) {
            ret+=",<span style='font-size: large'><b> "+inventoryText+"</b></span>";
            
        }
        if (chartText !== smallText){
                ret+=",<span style='font-size: medium'> "+smallText+"</span>"
            }
        ret="<p class='item' id='0'>"+ret+'</p>'
...
//        }
    ...
    function polarity(element,text){
    }
    function make5() {
        var table="<table style='width: 100%'><tr><th  style='width: 50%;'><button class='polarity'>Distracting</button></th><th style='width: 50%'><button class='polarity'>Uplifting</button></th></tr><tr><td colspan='2' style='border: 1px solid black;' id='frequency'><div id='regular0'></div></td></tr><tr><td colspan='2'  id ='inventory'></td></tr><tr><td colspan='2' style='border: 1px solid black;' id='intensity'><div id='intense0'></div></td></tr></table>";
        BERING.content = BERING.Command.execute("init");
        var itemSection = $("body").find("div.itemDisplay");
        itemSection.append($(table));
        itemSection.append($("<div id='item0' ></div>"));
        $('#item0').append($("<div id='regular0'></div>"));
        $("#regular0").css("background-color", "#bcecfe");
        $("#regular0").append($("<div style='text-align: center;' >FREQUENCY</div>"));
        $("#regular0").append($("<div class='rateRoc' id='0s'></div>"));
        makeRatePromo('#0s');
        $("#inventory").append($(fairRepresentation(0)));
        //$('#inventory').append($("<div id='intense0'></div>"));
        $("#intense0").css("background-color", "#c4d1e3");
        $("#intense0").append($("<div style='text-align: center;' >INTENSITY</div>"));
        $("#intense0").append($("<div class='rateRoc' id='0i'></div>"));
        
        
        //polarity($("#distracting"),"distracting");
        makeRatePromo2('#0i');
        ...
        $(".rateBut").on("ratingChoice:toggle", function () {
            ...
            if ($(this).is(".rateButOff")) {
                ...
                if ($('#0i').children().is(".intenseButOn")) {
                    console.log($('#0').text())
                    $('#0').addClass('on');
                }
            }
